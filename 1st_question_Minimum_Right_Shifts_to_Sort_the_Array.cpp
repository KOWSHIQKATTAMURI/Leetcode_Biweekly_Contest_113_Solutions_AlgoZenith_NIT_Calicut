...
public:
    int minimumRightShifts(vector<int>& nums){
        ...
        //if the number of instances is greater than one, that means there are more than one point where the present element is greater than its next element. So right shifting it does not make it sorrted.
        ...
        if(number_of_break_points == 0){
            return 0;
        }else if((number_of_break_points > 1) or (nums[n - 1] > nums[0])){
            return -1;
        }
